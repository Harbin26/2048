...
def get_max_score(state):
    ...
    max_score = 0
    for action in ALLOWED_ACTIONS:
        score, new_state = move_action(action, state)
        if score >= max_score:
            max_score = score

    return max_score

def get_best_next_score(state):
    """
    ...
    for temp_state in list_of_states:
        score = get_max_score(temp_state)
        if score >= best_score:
            best_score = score
...
def generate_random_next_state(current_score_list, state):
    ...
    if len(current_score_list) == 0:
        ...
        score, state = move_action(last_move, state)
        return score, last_move, add_2_or_4_grid(state, randomly_generate_2_4())
    ...
    for score, last_move, state in current_score_list:
        next_score = get_best_next_score(state)
        if score + next_score >= max_score_so_far:
            max_score_so_far = score + next_score
            best_move = last_move
            best_move_score = score
            best_move_state = state

    return best_move_score, best_move, add_2_or_4_grid(best_move_state, randomly_generate_2_4())
...
def play_2048_game():
    ...
    #max_tile_score = max(max(x) for x in state)
